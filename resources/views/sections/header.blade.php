...
<header x-data="{ mobileOpen: false }" class="relative top-0 z-50 bg-white shadow-md masthead fixed-top">
	...
	<div class="items-center justify-between hidden h-full py-4 px-12 mx-auto md:flex">
		...
		<div class="flex items-center gap-5 ml-6">
			...
			{{-- SEARCH --}}
			{{-- Pasek wyszukiwarki z live wynikami --}}
			<div x-data="{
      q: '',
      open: false,
      loading: false,
      results: [],
      rest: '{{ esc_url( rest_url('oos/v1/search-products') ) }}',
      search: _.debounce(async function() {
        const term = this.q.trim();
        if (term.length < 3) { this.results = []; this.open = false; return; }
        this.loading = true;
        try {
          const res = await fetch(`${this.rest}?q=${encodeURIComponent(term)}`);
          const data = await res.json();
          this.results = (data && data.items) ? data.items : [];
          ...
          this.loading = false;
				...
				<form method="get" action="{{ esc_url( home_url('/') ) }}"
					...
					<div class="bg-primary p-2 rounded-full ml-0.5">
						<svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
							<path d="M16.0792 15.4904L11.9482 11.4255C13.03 10.2502 13.6947 8.69586 13.6947 6.98545C13.6941 3.32658 10.6804 0.360901 6.96243 0.360901C3.24448 0.360901 0.230713 3.32658 0.230713 6.98545C0.230713 10.6443 3.24448 13.61 6.96243 13.61C8.56884 13.61 10.0422 13.0544 11.1995 12.1307L15.3466 16.2116C15.5486 16.4107 15.8767 16.4107 16.0788 16.2116C16.2813 16.0126 16.2813 15.6895 16.0792 15.4904ZM6.96243 12.5908C3.81664 12.5908 1.26648 10.0812 1.26648 6.98545C1.26648 3.88971 3.81664 1.38013 6.96243 1.38013C10.1082 1.38013 12.6584 3.88971 12.6584 6.98545C12.6584 10.0812 10.1082 12.5908 6.96243 12.5908Z" fill="white" />
						</svg>
					</div>
					<input
						type="search"
				...
				<div x-show="open"
					x-transition
					class="absolute right-0 mt-2 w-80 bg-white border rounded-lg shadow z-50">

					<template x-if="loading">
						<div class="p-3 text-sm text-gray-500">Szukam…</div>
					</template>

					<template x-if="!loading && results.length === 0 && q.length >= 3">
						<div class="p-3 text-sm text-gray-500">Brak wyników</div>
					</template>
						...
						<template x-for="item in results" :key="item.id">
							<li>
								<a :href="item.url" class="flex items-center gap-3 p-2 hover:bg-gray-50">
									<img :src="item.img" alt="" class="w-12 h-12 object-cover rounded">
									<div class="min-w-0">
										<div class="text-sm font-medium truncate" x-text="item.title"></div>
										<div class="text-xs text-gray-600" x-html="item.price"></div>
									</div>
								</a>
							</li>
						</template>
					...
					{{-- Link „Zobacz wszystkie” (opcjonalnie) --}}
					<div class="p-2 border-t text-right">
						<a :href="'{{ esc_url( home_url('/') ) }}?post_type=product&s=' + encodeURIComponent(q)"
							class="text-xs primary hover:underline">Zobacz wszystkie wyniki</a>
					</div>
				</div>
			</div>
